...
int main(void) {
  ...
  // calcul des résidus
  double *residual = malloc(2 * n * sizeof(double));
  ...
  */
  ...
  // valeur du résidu selon ZZ sur le bas
  int numberB[4] = {140, 141, 142, 143};
  double average_value_YY_bottom = 0;
  for (int i = 0; i < 4; i++) {
    average_value_YY_bottom += sigma_YY[numberB[i]];
  }
  average_value_YY_bottom /= 4.0;
  printf(" ==== Average value of YY residual on the bottom domain    :      %f\n", average_value_YY_bottom);
    ...
    for (mode; mode < 5; mode++) {
      ...
      while (glfwGetTime() - start < 1.2) {
        ...
        if (mode == 3) // affichage de sigma_YY (ZZ stress)
        {
          ...
          glColor3f(1.0, 0.0, 0.0);
          glfemMessage(theMessage);
          sprintf(theMessage, "simga_zz residuel moyen sur bottom domain : %f MPa", average_value_YY_bottom / 1.e6);  // valeur à changer après avoir calculé les résidus en zz
